 ...
 *  Copyright (C) 2022 John Stef <john@johnstef.com>
 *  Originally written Allan McRae <allan@archlinux.org>
 *
 ...
 *
 *  If you want to use it on your MacBook you should check that your 
 *  sensors are at the same place as mine.
 *
 *  20100516 - v1.0
 *  Notes: 
 ...
 *  20220208 - v1.2
 *  Notes: 
 *    Modified to work with MacBook Pro 8.1.
 *    Gets the average temperature from 3 sensors.
 *
 *  Tested models:
 *    MacBook Pro 8.1 13"  (i5 - 2011) 
 */
...
unsigned short get_temp()
{
  ...
  file=fopen("/sys/devices/platform/coretemp.0/hwmon/hwmon4/temp1_input", "r");
  fscanf(file, "%d", &t0);
  fclose(file);
  
  file=fopen("/sys/devices/platform/coretemp.0/hwmon/hwmon4/temp2_input", "r");
  fscanf(file, "%d", &t1);
  fclose(file);

  file=fopen("/sys/devices/platform/coretemp.0/hwmon/hwmon4/temp3_input", "r");
  fscanf(file, "%d", &t2);
  fclose(file);
  
  temp = (unsigned short)(ceil((float)(t0 + t1 + t2) / 3000.));
  return temp;
}
